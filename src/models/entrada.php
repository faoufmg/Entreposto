...
$stmt = mysqli_prepare($conn, "SELECT * FROM Entrada WHERE Codigo = ? AND Validade = ? AND Nome = ?");
mysqli_stmt_bind_param($stmt, 'sss', $Codigo, $Validade, $NomeTratado);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);

if (mysqli_num_rows($result) > 0) {
  $stmt = mysqli_prepare($conn, "UPDATE Entrada SET Quantidade = Quantidade + ?, QuantidadeInicial = QuantidadeInicial + ?, DataCad = ?, Usuario = ? WHERE Codigo = ? AND Validade = ? AND Nome = ?");
  mysqli_stmt_bind_param($stmt, 'iisssss', $Quantidade, $Quantidade, $DataCadastro, $Usuario, $Codigo, $Validade, $NomeTratado);
} else {
  $stmt = mysqli_prepare($conn, "INSERT INTO Entrada(Nome, Validade, DataCad, Descricao, Quantidade, Codigo, Local, QuantidadeInicial, Movimentacao, Usuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if (mysqli_stmt_execute($stmt)) {
  ...
  echo
    "<script>
      alert('Erro na consulta: . mysqli_error($conn)');
    </script>";
}
